...
**Inheritance declares a relationship**: *a Car IS A Vehicle.* Identity, fixed
forever in a family tree:
...
**Composition declares behaviour**: *a Car HAS an engine, so a Car CAN start.*
What it has and what it does - built from parts:
...
**Inheritance asks "what are you?" - answered once, forever.
Composition asks "what can you do?" - answered by whatever parts you hold.**

<!-- speaker_note: 'The framing to repeat: inheritance is an identity claim - I AM a vehicle - while composition is a capability claim - I HAVE an engine, I CAN start. Identity is decided once at declaration; capability is whatever parts you hold right now. Let the hybrid question land before revealing the second half - the room will feel the taxonomy break. Inheritance forces you to predict the future shape of the family tree, and you get it wrong: hybrid needs two branches at once, which is the diamond problem. Composition never asked the question - a hybrid is a car with a different engine. Also worth saying: the tree ALSO forced ElectricCar to inherit everything Car has, fields included; with composition the Car only holds what you put in it. Go removed inheritance so this is not even a choice you have to argue about in review.' -->
